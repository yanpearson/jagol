...
const doNextGeneration = (rulesTable, board) => {
	...
	const getPopulation = (boardWidth, boardHeight, rulesTable, stateArray) => {
		...
		const aliveCells = R.reduce((acc, value) => {
			...
			const isCellAlive = readArrayAtIndex(rulesTable)(cellState);
...
const createRulesTable = () => {
	...
	const buffer = new ArrayBuffer(513);

	const rulesTable = R.reduce((acc, value) => {
		const cellState = value;
		const neighborsCount = getNeighborsCount(cellState);
		// rule: if an alive cell is surrounded by 2 or 3 alive cells, it stays alive.
		if ((value & 16) && (neighborsCount === 2 || neighborsCount === 3)) {
			writeArrayAtIndex(acc)(value, ALIVE_STATE);
			return acc;
		}
		// rule: if a dead cell is surrounded by 3 alive cells, it reborns.
		if (!(value & 16) && (neighborsCount === 3)) {
			writeArrayAtIndex(acc)(value, ALIVE_STATE);
			return acc;
		}
		writeArrayAtIndex(acc)(value, DEAD_STATE);
		return acc;
	}, new Uint8Array(buffer), range(513));
